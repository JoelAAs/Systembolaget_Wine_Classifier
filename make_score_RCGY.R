make_score_RCGY <- function(wine_in){
  ...
  for(i in 1:length(regions)){
    tmp <- data.frame(regions[i],mean(wine_in$real_score[which(wine_in$simple_region == regions[i])]), stringsAsFactors = F)
    colnames(tmp) <- c("name", "score")
    table_out <- rbind(table_out,tmp)
  }
  
  for(i in 1:length(country)){
    tmp <- data.frame(country[i],mean(wine_in$real_score[which(wine_in$Ursprunglandnamn == country[i])], stringsAsFactors = F))
    colnames(tmp) <- c("name", "score")
    table_out <- rbind(table_out,tmp)
  }
  
  for(i in 1:length(grape)){
    tmp <- data.frame(grape[i],mean(wine_in$real_score[which(wine_in$class_grape == grape[i])], stringsAsFactors = F))
    colnames(tmp) <- c("name", "score")
    table_out <- rbind(table_out,tmp)
  }
  
  for(i in 1:length(year)){
    tmp <- data.frame(year[i],mean(wine_in$real_score[which(wine_in$Argang == year[i])], stringsAsFactors = F))
    colnames(tmp) <- c("name", "score")
    table_out <- rbind(table_out,tmp)
  }
